...
const AllOrder = () => {
    const { orders, loading, fetchOrdersForUser, downloadInvoice } = useOrderStore();
    ...
    return (
        <div className="min-h-screen bg-[#fcf7f8] flex flex-col items-center">
            <div className="container mx-auto px-4 py-16 mt-16">
                <motion.h1
                    className="text-4xl font-bold mb-8 text-[#A31621] text-center"
                ...
                {orders.length > 0 ? (
                        ...
                        <table className="min-w-full divide-y divide-gray-300">
                            <thead className="bg-[#A31621]">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase">Total Quantity</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase">Total Price</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase">Status</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase">Invoice</th>
                                </tr>
                            </thead>
                            <tbody className="bg-[#fcf7f8] divide-y divide-gray-300">
                                {orders.map((order, index) => {
                                    const productNames = order.products.map((p) => p.product?.name || "N/A").join(", ");
                                    ...
                                    return (
                                        <tr key={order._id} className="hover:bg-gray-100 transition">
                                            ...
                                            <td className="px-6 py-4 text-[#A31621] font-semibold">{productNames}</td>
                                            <td className="px-6 py-4 text-gray-700">{totalQuantity}</td>
                                            <td className="px-6 py-4 text-gray-700">Rs.{totalPrice.toFixed(2)}</td>
                                            <td className="px-6 py-4 text-[#A31621] font-semibold">{order.orderStatus}</td>
                    ...
                    <motion.div
                        className="flex flex-col items-center justify-center text-center py-10 bg-white rounded-lg shadow-md max-w-lg mx-auto"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.8 }}
                    >
                        <p className="text-lg text-gray-600 font-medium">No orders found.</p>
                        <p className="text-sm text-gray-500">It looks like you haven’t placed any orders yet.</p>
                    </motion.div>
                )}
            </div>
        </div>
